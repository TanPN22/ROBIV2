...
OpticalSensor_MeasureUsePollingMode(uint8_t irSensID)
{
	...
	switch (irSensID) {
		...
		case 1:
			ADC_RegularChannelConfig(IRx_SENSOR, ADC_Channel_1, 1, ADC_SampleTime_239Cycles5);
			break;

		case 2:
			ADC_RegularChannelConfig(IRx_SENSOR, ADC_Channel_2, 1, ADC_SampleTime_239Cycles5);
			break;

		case 3:
			ADC_RegularChannelConfig(IRx_SENSOR, ADC_Channel_3, 1, ADC_SampleTime_239Cycles5);
			break;

		case 4:
			ADC_RegularChannelConfig(IRx_SENSOR, ADC_Channel_4, 1, ADC_SampleTime_239Cycles5);
			break;
	...
	while (ADC_GetFlagStatus(IRx_SENSOR, ADC_FLAG_EOC) == RESET);

	// Read value
	result = ADC_GetConversionValue(IRx_SENSOR);
